...
const SERVER_URL = process.env.REACT_APP_SOCKET_URL || 'https://game-3ot9.onrender.com';
...
const useGameStore = create((set, get) => ({
  ...
  createRoom: (name) => {
    set({ playerName: name });
    get().socket.emit('create-room', { playerName: name });
  },

  joinRoom: (roomId, name) => {
    set({ playerName: name });
    get().socket.emit('join-room', { roomId, playerName: name });
  },

  startGame: (category, rounds) => {
    const { roomId } = get();
    get().socket.emit('start-game', { roomId, category, rounds });
  },

  nextRound: (category) => {
    const { roomId } = get();
    get().socket.emit('next-round', { roomId, category });
  },

  submitAnswer: (answer) => {
    const { roomId } = get();
    get().socket.emit('submit-answer', { roomId, answer });
  },

  startVoting: () => {
    const { roomId } = get();
    get().socket.emit('start-voting', { roomId });
  },

  submitVote: (votedPlayerId) => {
    const { roomId } = get();
    get().socket.emit('submit-vote', { roomId, votedPlayerId });
  },

  playAgain: () => {
    const { roomId } = get();
    get().socket.emit('play-again', { roomId });
  },
